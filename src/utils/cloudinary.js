...
const deleteFromCloudinary = async (publicOldFilePath) => {
  try {
    if (!publicOldFilePath) return null;
    await cloudinary.uploader.destroy(publicOldFilePath);
  } catch (error) {
    console.error("Error removing existing file from cloudinary", error);
    return null;
  }
};
